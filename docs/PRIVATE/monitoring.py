...
def modbus_monitor():
    ...
    for debug_data in debug_vars:
        if (debug_data.location.find("IX")) > 0:
            ...
            result = mb_client.read_discrete_inputs(
                int(mb_address[0]) * 8 + int(mb_address[1]), 1
            )
            ...
            if (
                (debug_data.type == "SINT")
                or (debug_data.type == "INT")
                or (debug_data.type == "DINT")
            ):
                # signed integer
            ...
            if (
                (debug_data.type == "USINT")
                or (debug_data.type == "UINT")
                or (debug_data.type == "UDINT")
            ):
                # unsigned integer
                float_pack = pack(">HH", result.registers[0], result.registers[1])
                debug_data.value = unpack(">I", float_pack)[0]
